    ...
    return (
        <div class="home__filter">
            ...
            <div class="home__filter-sort">
                ...
                <button className={`home__filter-sort-btn ${value.sort == -1 ? "home__filter-sort-btn-select" : ""}`} name="sort"  value = {-1} onClick={onClickSort}>Giá giảm dần
                {value.sort == -1 ? <span class="home__filter-sort-btn-check">✓</span> : ''}
                </button>
                <button className={`home__filter-sort-btn ${value.sort == 1 ? "home__filter-sort-btn-select" : ""}`} name="sort" value ={1}  onClick={onClickSort}>Giá tăng dần 
                {value.sort == 1 ? <span class="home__filter-sort-btn-check">✓</span> : ''}

                </button>
            </div>
            <div class="home__filter-price">
                <input type="text" id="" class="typing-1" value={value.min} placeholder="Giá thấp nhất" name="min" onChange={onChangeValue} />
                <span class="space">-</span>
                <input type="text" id="" class="typing-2" value={value.max} placeholder="Giá cao nhất" name="max" onChange={onChangeValue} />
                <button type="submit" class="home__filter-sort-btn search-item" onClick={onClickFilter}>Tìm</button>
            </div>
        </div>
    );
};
